...
class YoloTrt(Node):
    def __init__(self):
        ...
        self.pub_image_1 = self.create_publisher(Image, 'trt_image_1', 10)
        self.pub_image_2 = self.create_publisher(Image, 'trt_image_2', 10)
        self.pub_image_3 = self.create_publisher(Image, 'trt_image_3', 10)
        self.pub_image_4 = self.create_publisher(Image, 'trt_image_3', 10)
        timer_period = 0.025  # seconds
        self.timer = timer_period
        self.sub_img1 = self.create_subscription(Image, 'img_1',self.timer_callback_1,1)
        self.sub_img2 = self.create_subscription(Image, 'img_2',self.timer_callback_2,1)
        self.sub_img3 = self.create_subscription(Image, 'img_3',self.timer_callback_3,1)
        self.sub_img4 = self.create_subscription(Image, 'img_4',self.timer_callback_4,1)
        self.sub_img1
    ...
    def timer_callback_1(self, image:Image):
        #self.yolov5_wrapper = yolov5_wrapper
        #self.cap = cv2.VideoCapture(0)
    ...
    def timer_callback_3(self, image:Image):
        categories = self.categories
        '''while (True):
            ret,frame = self.cap.read()
            result, use_time = self.yolov5_wrapper.infer(frame, categories)
        ...
        try:
            image_raw = self.br.imgmsg_to_cv2(image)
            result, use_time = self.yolov5_wrapper.infer(image_raw, categories)
            self.pub_image_3.publish(self.br.cv2_to_imgmsg(result))
            cv2.imshow('Back', result )
            cv2.waitKey(1)
        except KeyboardInterrupt:
            self.yolov5_wrapper.destroy()
            exit(0)
    def timer_callback_4(self, image:Image):
        #self.yolov5_wrapper = yolov5_wrapper
        #self.cap = cv2.VideoCapture(0)
        categories = self.categories
        '''while (True):
