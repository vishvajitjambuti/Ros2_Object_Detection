...
class inferThread(threading.Thread):
    ...
    def run(self):
        ...
        while(True):
            ...
            print(frame.shape)
            result, use_time = self.yolov5_wrapper.infer(frame, self.categories)
...
class YoloTrt(Node):
    ...
    def timer_callback(self):
        thread1 = inferThread(self.yolov5_wrapper, self.categories)
        thread1.start()
        
        thread1.join()
        self.pub_image.publish(self.br.cv2_to_imgmsg(thread1.result))
